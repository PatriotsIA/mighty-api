...
type Geography = { stateSlug: string; countySlug?: string; countyName?: string; countySlugs?: string[] };
...
export function geographyIssue(value: Geography): string | undefined {
  ...
  if (!value.countySlug && value.countyName) return "A county name requires a county selection.";
  ...
  if (value.countySlugs?.some((slug) => !counties!.has(slug))) return "Every covered county must belong to the selected state.";
  if (!value.countySlug) return undefined;
