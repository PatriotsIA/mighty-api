...
The current office-driven form uses `localized-republican-2026-09`, whose wording follows the candidate's selected state and county. The API also accepts the original `texas-republican-2026-09` edition. The frontend keeps the original wording when displaying saved responses from that edition. Private submitter details may include optional boolean `interviewRequested` and `advertisingRequested` fields. They remain outside public candidate projections; checked interests are included in staff notification emails. Deploy this schema support before releasing the updated form. No migration is required.
